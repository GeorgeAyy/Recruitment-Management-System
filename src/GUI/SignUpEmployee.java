 ...
 * @author fatim
 */
public class SignUpEmployee extends Scene {

    public SignUpEmployee(StackPane root, double d, double d1, Stage primaryStage) {
        super(root, d, d1);
        ...
        Label signin = new Label("Sign Up as an Employee");
            ...
            public void handle(ActionEvent event) {
                try {
                    ...
                    if (pass.equals(confirmedpass)) {
                        if (reg.checkemployee(user, pass) == false) {
                            reg.regAsEmployee(user, pass);
                            primaryStage.setScene(new SigninEmployee(new StackPane(), 1200, 700, primaryStage));
                        } else {
                            error.setText("Corporate name already taken");
                        }
                    } else {
